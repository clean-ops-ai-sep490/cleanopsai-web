  ...
  const handleUpdate = async (data: any) => {
    try {
      await updateMutation.mutateAsync({
        id: schedule!.id,
        data: data as UpdateTaskScheduleData,
      });
      setIsEditing(false);
      onSuccess?.();
      toastUtils.success("Cập nhật lịch trình thành công");
    } catch (error) {
      ...
      <SheetContent className="sm:max-w-2xl p-0 flex flex-col h-full border-l-0 shadow-2xl">
        <div className="p-6 pb-4 flex items-center justify-between bg-white border-b sticky top-0 z-10">
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              ...
              >
                {schedule?.recurrenceType || initialSchedule.recurrenceType}
              </Badge>
            </div>
          ...
          <div className="flex gap-2">
            ...
            {isEditing && (
              <Button
                ...
                className="rounded-xl h-9 px-4 font-bold text-xs text-slate-500"
              >
                Hủy chỉnh sửa
              </Button>
            )}
        ...
        <ScrollArea className="flex-1 min-h-0">
          {isFetching && !fullSchedule ? (
            ...
            <div className="p-6">
              <TaskScheduleForm
                initialData={schedule as any}
                onSubmit={handleUpdate}
                isSubmitting={updateMutation.isPending}
                submitButtonText="Cập nhật thay đổi"
              />
            </div>
          ) : schedule ? (
              ...
              {schedule.recurrenceConfig && (
                <section className="space-y-4">
                  <div className="flex items-center gap-2 text-slate-900">
                    ...
                    <h3 className="font-bold text-sm">Cấu hình thời gian</h3>
                  </div>
